...
public class ReceiptGenerateService {
    ...
    public ReceiptGenerateService(@Autowired SpringTemplateEngine templateEngine,
    ...
    public String createReceipt(OrderDto order){
        OrderReceipt orderReceipt = orderReceiptRepository.findById(order.getId())
                .orElse(OrderReceipt.builder().build());
        Context context = new Context();
        context.setVariable("order", order);
        context.setVariable("now", LocalDateTime.now());
        String html = templateEngine.process("receipt", context);
        byte[] pdfBytes = convertHtmlToPdf(html);
        String fileName = "receipt-" + order.getId() + ".pdf";
        ByteArrayResource pdfResource = new ByteArrayResource(pdfBytes, fileName);
